(function ($) {
	...
	Drupal.behaviors.iaa_notices_warning = function(context) {
		$('._iaa_notices_warning_questions').submit(function(){
			var checked = true;
			var form = this;
			$(form).children('._mandatory').each(function(){
				if (!$(this).attr('checked')) {
					checked = false;
				}
			});
			if (checked) {
				if ($(form).children('._iaa_notices_warning_questions_uid').val() == 0) {
					iaa_notices_warning_form_login(form);
				}
				else {
					// on affiche le message en Ajax
					iaa_notices_warning_get_message(form);
				}
			}
			else {
				alert(Drupal.t("You have to check all the fields."));
			}
			return false;
		});
